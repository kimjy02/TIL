...
def get_road_move_time(row, col):
    ...
    while queue:      # 후보군이 있는 동안
        ...
        for k in range(4):
            ...
            # 그럴려면 리스트의 범위를 벗어나지 않아야 한다.
            # 그리고, 이전에 방문한 적 없어야 한다.     -1로 초기화 해두었음
            # 그리고, 그 위치가 길 이어야 한다. -> 1은 길 / 0은 벽
            if 0 <= nx < N and 0 <= ny < M and distance[nx][ny] == -1 and data[nx][ny]:
                ...
                if nx == N-1 and ny == M-1:    # 도착지다.
                    return
    # 모든 후보군을 다 탐색했지만, return돼서 함수가 종료된 적이 없이
...
# 방문 표시를 할 거다. -> 우리의 최종 목적이 무엇이냐?
# 해당 위치까지 도달하는 데 걸린 비용이 얼마인지를 기록!
