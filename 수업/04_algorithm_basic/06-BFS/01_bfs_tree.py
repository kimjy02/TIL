def BFS(root_node):
    '''
    :param root_node: 너비우선 탐색을 시작할 서브 트르의 루트
    :return: 완성된 경로
    '''
    ...
    while data_structure:     # 후보군이 남아 있으면 계속 조사
        # node = data_structure.pop()     # LIFO 해버려서 DFS가 되어버림
        node = data_structure.pop(0)      # FIFO
        result.append(node)
        # 내 자식 노드들 (인접 노드들을 모두 찾아서)
        for child in graph.get(node, []):     # get 메서드 : key값(node)을 받고 key가 없으면 [](빈 리스트) 반환
