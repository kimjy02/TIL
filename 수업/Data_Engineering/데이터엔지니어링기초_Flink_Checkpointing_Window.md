...
- 상태의 유형
  - '키'가 있는(Keyed) 상태 : 특정 키와 연관된 상태로, 각 키마다 별도의 상태를 유지
...
- 사용이유(길이가 1시간인 윈도우에서 최대값 구하기)
  ...
  - 윈도우 크기는 1시간으로 유지 → 여전히 1시간 내에서 트리거가 실행되지만, 계산은 최근 10분 데이터만 사용하는 효과가 발생.
...
- 지연 도착 데이터 처리
  ...
  - Allowed Lateness(Duration)을 설정하면, 해당 기간 내에 도착한 늦은 이벤트는 다시 그 윈도우를 열어서 계산을 반영
