# 데이터엔지니어링 활용 - 배치처리 워크플로우 구축 (Airflow + Spark)
...
### 2) 퀴즈
#### 다음 중 Apache Spark의 배치 처리와 실시간 처리에 대한 설명으로 옳은 것은?
A. 실시간 처리는 데이터를 모아서 한꺼번에 처리하는 방식이다.
B. 배치 처리는 지연 시간이 거의 없고, 데이터가 들어오자마자 처리된다.
**C. 실시간 처리는 데이터가 생성되자마자 처리되며, 지연 시간이 짧다.**
D. 배치 처리는 실시간 데이터 분석에 가장 적합하다.

### 3) Airflow와 Spark를 활용한 데이터 처리
#### Apache Spark
...
1. 데이터를 `parallelize` 또는 외부 소스를 불러와 RDD를 생성
...
2. 생성된 DataFrame에서 `select()`, `filter()`, `groupBy()` 등의 Transformation 연산 적용
...
#### Airflow에서 Spark를 활용
- 단순 Python 코드가 처리하기 어려운 대규모 데이터 처리
...
### 5) Airflow에서 배치 작업 최적화
#### 병렬 실행 및 성능 최적화 방법
![alt text](image-184.png)

## 2. Airflow Decorator
### 1) Airflow에서 배치 작업 최적화
#### Airflow Decorator
...
## 3. SparkSubmitOperator
### 1) DAG에서 SparkSubmitOperator 활용법
#### SparkSubmitOperator 개요 및 역할
- Apache Airflow에서 Spark 애플리케이션을 실행하기 위한 전용 연산자
- Spark 클러스터(YARN, Kubernentes 등)에 .py, .jar, .scala 파일 등을 제출(submit)
...
- `conn_id` : 클러스터 연결 정보 (예 : spark_default)
- `name` : 작업 이름
