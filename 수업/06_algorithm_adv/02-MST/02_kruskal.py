...
def mst_kruskal(vertices, edges):
    '''
        ...
        4. 2와 3을 선택된 간선이 (n-1)개가 될 때까지 반복한다.
    '''
    mst = []     # 최소 신장 트리를 구성하는 간선 정보를 담을 리스트
    # 가중치를 기준으로 오름차순으로 정렬한다.
    # 간선 정보의 구성 [[시작노드, 종료노드, 가중치], ]
    edges.sort(key = lambda x: x[2])
    print(edges)
    ds = DisjointSet(vertices)  # disjoint-set 생성하기
    ...
    for edge in edges:
        ...
        '''
            가중치에 대한 정보는 처음 오름차순 과정에서 역할을 다 했고
            이제 우리가 할 일은
            s와 e가 같은 집합에 속해있는지 판별하고,
            만약 같은 집합에 속해있지 않은 노드들이라면,
            합칠 수 있음!
        '''
        # 두 노드가 다른 집합에 속한 경우, union
