...
# 1번
def Adults(users):
    return [user for user in users if user.get('age') >= 18]
...
def Adult_Active_Users(users):
    return [user for user in users if (user.get('age') >= 18 and user.get('is_active') == True)]

# 4번
print('Adults:', Adults(users))

print('Active Users:', Active_Users(users))

print('Adult Active Users:', Adult_Active_Users(users))